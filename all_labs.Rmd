...
- the normal cdf cannot approximately binom cdf well at the jump points, bc its continuous whereas binom is discrete, but it approximates it very well half-way between these points of the binom cdf
...
**c)** Repeat (a) and (b) for the sum of lognormal(0,1) distributions (use `rlnorm(n, meanlog = 0, sdlog = 1)`). In this case, E(Xi ) = exp(1/2), Var(Xi ) = (exp(1) − 1) exp(1).
...
legend("topright", legend = c("Chi-squared distribution with df=49", "N(49, 2*49)"),
...
1 - pnorm(54, mean = n-1, sd = (2*(n-1))**0.5) 
```
